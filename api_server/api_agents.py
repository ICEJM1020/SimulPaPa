...
@abp.route(f"/agent/<username>/<agent_id>/steps/<date>", methods=["POST", "GET"])
def fetch_steps(username, agent_id, date):
    status, user_list = check_user(username=username)
    if status:
        _uuid = user_list[username]
        if USER_POOL.exist(_uuid):
            return_body = {
                "status" : True,
                "data" : USER_POOL.fetch_agent_steps(_uuid, int(agent_id), date),
                "message" : "",
            }
        else:
            return_body = {
